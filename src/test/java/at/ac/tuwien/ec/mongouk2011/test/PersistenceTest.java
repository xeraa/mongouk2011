...
public class PersistenceTest {
	...
	/**
	 * Get our persistence implementation and ensure it's not null.
	 * Were not inserting any test data as there is no dataset we can easily use for all test cases.
	 */
	@Before
	...
	/**
	 * After finishing the test, clean up the database.
	 * This is important to allow multiple test runs in combination with unique key constraints.
	 */
	@After
	...
	public void persistCompanyEntity(){
		CompanyEntity company = new CompanyEntity("Test company", null, null, null, "http://www.test.com",
				"foobar@test.com");
	...
	public void getAllCompanies(){
		assertEquals("Right at the start no companies should be found", 0, persistence.getAllCompanies().size());
		CompanyEntity company1 = new CompanyEntity("First company", null, null, null, "http://www.test1.com",
				"foobar@test1.com");
		persistence.persistCompanyEntity(company1);
		CompanyEntity company2 = new CompanyEntity("Second company", null, null, null, "http://www.test2.com",
				"foobar@test2.com");
		persistence.persistCompanyEntity(company2);
		assertEquals("After persisting two companies they should be found", 2,
				persistence.getAllCompanies().size());
		persistence.clearData();
		assertEquals("After clearing no companies should be found again", 0,
				persistence.getAllCompanies().size());
	}
	...
	public void findByEmail(){
		...
		assertEquals("The right address should find the correct user", "Philipp",
	...
	public void findCompanyByCountry(){
		AddressEntity address = new AddressEntity("Street", "1234", "London", "UK", AddressType.WORK);
		BankConnectionEntity bankConnection = new BankConnectionEntity("Account owner", "1234567890",
				"11111", "222222222222", "333333333", "UK");
		CompanyEntity company = new CompanyEntity("Test company", null, null, null, "http://www.test.com",
				"foobar@test.com");
		...
		assertEquals("Equals is case sensitive, use regular expression when needed", 0,
				persistence.findCompanyByCountry("uk").size());
	...
	public void findCompanyEmployees(){
		...
		persistence.persistWorkerEntity(worker2);
		assertEquals("One employee should be working for the company", 1,
				persistence.findCompanyEmployees(company.getName()).size());
		assertEquals("The worker's name should be Philipp", "Philipp",
				persistence.findCompanyEmployees(company.getName()).get(0).getFirstname());
		persistence.workingFor(worker2, company);
		persistence.workingFor(manager, company);
		assertEquals("Three employees should be working for the company", 3,
				persistence.findCompanyEmployees(company.getName()).size());
	}
	...
	public void uniqueness(){
		...
		assertEquals("When adding a second user with the same email address, the first one should be preserved",
	...
	public void concurrency(){
		...
		assertEquals("When doing two concurrent edits, only the first should go through",
				"Paul", persistence.getAllEmployees().get(0).getFirstname());
	...
	public void objectsPrimitives(){
		...
		assertNull("Objects can have null values", persistence.getAllManagers().get(0).getApproveFunds());
		...
		assertFalse("Primitives can't have null values (initialized to false)",
				persistence.getAllManagers().get(0).isApproveHires());
	}
